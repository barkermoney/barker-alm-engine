...
| `app/src/*.ts` — Sep 11 changes, `.github/workflows/pages.yml` | **AI-written**, in the author's session after the author decided to publish the dashboard: the GitHub Pages workflow with its hourly snapshot refresh, and two fixes found by loading the published page — live reads no longer blocked by a rate-limited catch-up, and Arc reads batched into one request. |
...
| `README.md` Arc section, `docs/schedule.md` prize table — Sep 12 | **AI-edited** in a scheduled assistant session without the author present, after re-reading Arc's prize page and finding its tracks restructured: the bounty named is now the one that exists, and the mainnet deployment is described as required rather than optional. Pending the author's review. |
