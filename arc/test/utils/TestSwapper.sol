...
contract TestSwapper is IUnlockCallback {
    ...
    {
        bytes memory res = manager.unlock(abi.encode(msg.sender, key, zeroForOne, amountSpecified, sqrtPriceLimitX96));
        delta = abi.decode(res, (BalanceDelta));
    }

    function unlockCallback(bytes calldata raw) external returns (bytes memory) {
        require(msg.sender == address(manager), "mgr");
        (address payer, PoolKey memory key, bool zeroForOne, int256 amt, uint160 lim) =
            abi.decode(raw, (address, PoolKey, bool, int256, uint160));

        BalanceDelta delta = manager.swap(key, SwapParams(zeroForOne, amt, lim), "");
